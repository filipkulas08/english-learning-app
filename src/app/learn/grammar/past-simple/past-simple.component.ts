...
@Component({
  ...
  styleUrls: ['./past-simple.component.css']
})
export class PastSimpleComponent implements OnInit {
  user: firebase.User;
  activeUser:any =[];
  answer1:string;
  answer2:string;
      ...
      this.service.activeUser(this.user.uid).subscribe(data =>{
        this.activeUser = data.map(e =>{
          return{
            ...
            test: e.payload.doc.data()['pastSimple']
          };
        })
  ...
  checkTest(){
    ...
    if(this.answer2 === "visited"){
      this.points=this.points+1;
    }
    if(this.answer3 === "cost"){
      this.points=this.points+1;
    }
  ...
  test1Success(){
    let Record = {};
    Record['pastSimple'] = true;
    this.service.updateUser(this.user.uid,Record);
    }

  test1Miss(){
    let Record = {};
    Record['pastSimple'] = false;
    this.service.updateUser(this.user.uid,Record);
    }
}
