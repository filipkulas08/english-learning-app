...
@Component({
  ...
  styleUrls: ['./present-simple.component.css']
})
export class PresentSimpleComponent implements OnInit {
  user: firebase.User;
  activeUser:any =[];
  answer1:string;
  answer2:string;
      ...
      this.service.activeUser(this.user.uid).subscribe(data =>{
        this.activeUser = data.map(e =>{
          return{
            ...
            test: e.payload.doc.data()['presentSimple']
          };
        })
  ...
  checkTest(){
    if(this.answer1 === "listen"){
      this.points=this.points+1;
    }
    if(this.answer2 === "realise"){
      this.points=this.points+1;
    }
    if(this.answer3 === "do"){
      this.points=this.points+1;
  ...
  test1Success(){
    let Record = {};
    Record['presentSimple'] = true;
    this.service.updateUser(this.user.uid,Record);
    }

  test1Miss(){
    let Record = {};
    Record['presentSimple'] = false;
    this.service.updateUser(this.user.uid,Record);
    }
