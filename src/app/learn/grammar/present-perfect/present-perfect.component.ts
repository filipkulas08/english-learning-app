...
@Component({
  ...
  styleUrls: ['./present-perfect.component.css']
})
export class PresentPerfectComponent implements OnInit {
  user: firebase.User;
  activeUser:any =[];
  answer1:string;
  answer2:string;
      ...
      this.service.activeUser(this.user.uid).subscribe(data =>{
        this.activeUser = data.map(e =>{
          return{
            ...
            test: e.payload.doc.data()['presentPerfect']
          };
        })
  ...
  checkTest(){
    if(this.answer1 === "has been"){
      this.points=this.points+1;
    }
    if(this.answer2 === "Have" && this.answer3 === "seen"){
      this.points=this.points+1;
    }
    if(this.answer4 === "has" && this.answer5 === "slain"){
      this.points=this.points+1;
    }
  ...
  test1Success(){
    let Record = {};
    Record['presentPerfect'] = true;
    this.service.updateUser(this.user.uid,Record);
    }

  test1Miss(){
    let Record = {};
    Record['presentPerfect'] = false;
    this.service.updateUser(this.user.uid,Record);
    }
